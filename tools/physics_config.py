...
def throat(head_world, mouth_local):
    """THROAT in a skeleton's HEAD frame: the mouth [Mouth] places there, plus each offset (world axes),
    back into HEAD's frame (bone_table's formula)."""
    hr, ht, hs = head_world
    mouth = [ht[i] + v * hs for i, v in enumerate(zb.apply(hr, list(mouth_local)))]
    out = []
    for off in pd.THROAT:
        w = [mouth[i] + off[i] - ht[i] for i in range(3)]
        out.append([v / hs for v in zb.apply(zb.transpose(hr), w)])
    return out


def aim_keys(pelvis_world, head_f=None, head_m=None):
    ...
    for sex, head, key in (('female', head_f, 'throatF'), ('male', head_m, 'throatM')):
        if head is not None:
            mouth = (MOUTH[f'{sex}X'], MOUTH[f'{sex}Y'], MOUTH[f'{sex}Z'])
            keys[key] = ';'.join(fmt(q, 3) for q in throat(head, mouth))
    return keys


def anatomy_ini(pelvis_world, head_f=None, head_m=None):
    ...
    lines += ['', '; the penis finds its opening (A-28): openings in Pelvis_skin\'s frame, angles in degrees',
              '[Aim]'] + [f'{k}={v}' for k, v in aim_keys(pelvis_world, head_f, head_m).items()]
...
def main():
    ...
    # the throat in each skeleton's own HEAD frame: the women's (ours) and the men's (ZeX's)
    ours = {'ocbp.ini': anatomy_ini(bind[PARENT], women_world['HEAD'], bind['HEAD']),
